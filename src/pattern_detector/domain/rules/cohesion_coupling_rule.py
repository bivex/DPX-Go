"""Go High Cohesion / Low Coupling Rule."""
...
class HighCohesionLowCouplingRule(BasePatternRule):
    """Detects High Fan-Out (High Coupling) in Go structs."""
    ...
    def detect(self, model: CodeModel) -> list[Detection]:
        ...
        for st in model.all_structs():
            custom_types = set()
            for f in st.fields:
                t = f.type_str.lstrip("*").lstrip("[]")
                if t and (t[0].isupper() or "." in t) and t not in ("string", "int", "int64", "bool", "time.Time", "sync.Mutex", "sync.RWMutex"):
                    custom_types.add(t)
