"""Go Keep It Simple, Stupid (KISS) Rule (cyclomatic complexity & parameter lists)."""
    ...
    def detect(self, model: CodeModel) -> list[Detection]:
        ...
        for fn in model.all_functions():
            ...
            if len(fn.params) >= 5:
                evidences.append(
                    Evidence(
                        description=f"KISS Violation (Long Parameter List): Function '{fn.name}' takes {len(fn.params)} parameters; consider a Config struct or Functional Options",
                        weight=0.70,
